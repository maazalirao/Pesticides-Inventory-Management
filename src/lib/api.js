...
// Create user in our database after Clerk signup
export const createUser = async (clerkUser) => {
  try {
    ...
    // Format the user data for our backend
    const userData = {
    ...
    const response = await api.post("/users/clerk", userData);
...
export const addBatchToInventoryItem = async (id, batchData) => {
  try {
    ...
    throw (
      error.response?.data?.message || "Failed to add batch to inventory item"
    );
  }
};

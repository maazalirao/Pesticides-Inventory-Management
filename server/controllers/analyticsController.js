...
const getCustomerSegments = asyncHandler(async (req, res) => {
  ...
  if (ordersByCustomerType.length === 0) {
    ...
    const total = ordersByCustomerType.reduce((sum, type) => sum + type.total, 0);
...
const getSalesForecast = asyncHandler(async (req, res) => {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const actualData = monthlySales.find(item => 
...
const getExpiringProducts = asyncHandler(async (req, res) => {
  ...
  res.json(inventoryWithExpiringBatches.map(item => ({
    id: item._id,
    name: item.name,
...
const getRecentSales = asyncHandler(async (req, res) => {
  ...
  const formatDate = (date) => {
    ...
    if (orderDate >= today) {
      ...
      return orderDate.toLocaleDateString('default', { month: 'short', day: 'numeric', year: 'numeric' });
    }
  };
